...
GAN_database_folder_path='./database1/database/'
GAN_calculation_folder_path='./calculation_Encoder/'

##### 1. generate lattice and sites graph

#### 1.1. generate 223d lattice voxel graph
#print('generating 3d lattice voxel graph')
#dt.generate_lattice_graph(lattice_graph_path=GAN_calculation_folder_path+'original_laettice_graph/',atomlisttype='specified',a_list=['V'],data_path=GAN_database_folder_path+'geometries/',data_type='vasp')
...
#dt.generate_sites_graph(sites_graph_path=GAN_calculation_folder_path+'original_sites_graph/',atomlisttype='specified',a_list=['V','O'],data_path=GAN_database_folder_path+'geometries/',data_type='vasp')
...
#import prepare.lattice_autoencoder_plot as la #comment: lattice_autoencoder has many similar functions as sites_autoencoder, so need to be imported every time used
#la.lattice_autocoder(lattice_graph_path=GAN_calculation_folder_path+'original_laettice_graph/',encoded_graph_path=GAN_calculation_folder_path+'original_encoded_lattice/',model_path=GAN_calculation_folder_path+'model/')
...
#import prepare.sites_autoencoder_plot as sa
...
print('improved lattice autoencoder running:')
import prepare.Improved_lattice_autoencoder_plot as la #comment: lattice_autoencoder has many similar functions as sites_autoencoder, so need to be imported every time used
la.lattice_vae(lattice_graph_path=GAN_calculation_folder_path+'original_laettice_graph/',encoded_graph_path=GAN_calculation_folder_path+'original_encoded_lattice/',model_path=GAN_calculation_folder_path+'model/')
...
import gan.ccdcgan1 as gan1
ccdcgan = gan.CCDCGAN()
ccdcgan.train(epochs = 10000,batch_size=128, save_interval=5000,GAN_calculation_folder_path=GAN_calculation_folder_path,X_train_name='train_X.npy')

#xccdcgan = gan1.CCDCGAN()
#xccdcgan.train(epochs = 10000,batch_size=128, save_interval=5000,GAN_calculation_folder_path=GAN_calculation_folder_path,X_train_name='train_X.npy')
